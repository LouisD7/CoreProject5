...
class get_patient_details():
    ...
    def get_patient_data(self):
        try:
            ...
            return {
                'statusCode': 400,
                'body': json.dumps('No Patient ID in request')
            }
        try:
            if len(patient_id) != 6:
                return {
                    'statusCode': 400,
                    'body': json.dumps('Invalid patient ID Incorrect length')
                }
        except TypeError:
            return {
                    'statusCode': 400,
                    'body': json.dumps('Invalid patient ID check format of patient ID')
                }
